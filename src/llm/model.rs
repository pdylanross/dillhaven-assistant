use crate::app::candle::get_device;
use crate::app::huggingface::{load_json_file_as_async, HuggingFaceApiManger, SafeTensorsIndex};
use crate::llm::token_stream::TokenizerHandler;
use anyhow::Result;
...
mod test {
    ...
    #[tokio::test]
    pub async fn test_model_init_v2_2b() {
        let model = model_init(ModelVersion::InstructV2_2B).await;
    ...
    #[tokio::test]
    pub async fn test_model_init_v2_9b() {
        model_init(ModelVersion::InstructV2_9B).await;
    }
    ...
    async fn prompt_model_and_get_response(model: &Model, prompt: &str, prompted: bool) {
        info!("prompt: {}", prompt);
        let prompt = format!(
            "You are evaluating a conversation with a human. \
             There are many people in this conversation, but they are not labeled. \
             One of the participants name is dillhaven. \
             Determine if the following statement is addressed to dillhaven.\
             Answer with Yes or No, then a detailed explanation as to why you made your determination.\n\n
            \
            Is the following statement addressing dillhaven?\n\nStatement: {}",
            prompt
        );
        let resp = model.prompt(prompt).expect("failed to prompt");
        let resp = read_output_to_end(resp).await;

        let lowered = resp.to_lowercase();
        let result = lowered.starts_with("yes");
    ...
    #[tokio::test]
    pub async fn test_model_init_v3_1b_technical_prompt() {
        let model = model_init(ModelVersion::InstructV3_1B).await;

        prompt_model_and_get_response(&model, "What is the purpose of this project?", false).await;
        prompt_model_and_get_response(&model, "What is the meaning of life?", false).await;
        prompt_model_and_get_response(&model, "What is the meaning of chicken?", false).await;
        prompt_model_and_get_response(&model, "Hey jerry is the chicken done yet?", false).await;
        prompt_model_and_get_response(
            &model,
            "Where did I leave the god damn kitchen knives",
            false,
        )
        .await;
        prompt_model_and_get_response(&model, "Yo dillhaven", true).await;
        prompt_model_and_get_response(&model, "hey dillhaven", true).await;
        prompt_model_and_get_response(&model, "whats up dillhaven", true).await;
        prompt_model_and_get_response(&model, "Hello dillhaven", true).await;
        prompt_model_and_get_response(&model, "Hello dillhaven", true).await;
        prompt_model_and_get_response(&model, "Hello dillhaven", true).await;
        prompt_model_and_get_response(&model, "Hello dillhaven", true).await;
        prompt_model_and_get_response(&model, "Hello dillhaven", true).await;
